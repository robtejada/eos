...
Core/mantle-style wrapper around the iron2_eos (liquid iron, Zhang et al.)
tables.

Exposes the same API used by mgsio3_comb_eos.py, mg2sio4_aneos_eos.py,
aqua_core_eos.py, and iron_comb_eos.py so that this class can be used
as either a mantle_eos or core_eos option in the evolution code.
...
Units:
    ...
    S   : erg/g/K       (PT outputs)
          k_B/baryon    (SP inputs, matching the convention)
    U   : erg/g
    Cp  : erg/g/K
    Cv  : erg/g/K
    alpha : 1/K
...
try:
    from eos import iron2_eos
except ImportError:
    import iron2_eos


class IRON2_CORE_EOS:
    """
    Liquid iron (Zhang et al.) wrapper with the linear-unit interface used
    by the mantle and core EOSes.

    The underlying iron2_eos module stores tables in log-space:
    ...
    def __init__(self):
        ...
        # Latent heat of fusion for iron (Anderson & Duba 1997)
        self.L = float(1.2e6 * (u.J / u.kg).to("erg/g"))

        # ---- grab the underlying RGIs from iron2_eos ----
        # PT table
        self._logrho_rgi_pt = iron2_eos.logrho_rgi_pt
        self._s_rgi_pt = iron2_eos.s_rgi_pt
        self._logu_rgi_pt = iron2_eos.logu_rgi_pt
        self._cp_rgi_pt = iron2_eos.cp_rgi_pt
        self._cv_rgi_pt = iron2_eos.cv_rgi_pt

        # SP table
        self._logrho_rgi_sp = iron2_eos.logrho_rgi_sp
        self._logt_rgi_sp = iron2_eos.logt_rgi_sp
        self._logu_rgi_sp = iron2_eos.logu_rgi_sp

        # Axis ranges (in log10 dyn/cm^2)
        self._logp_min_pt = float(iron2_eos.logpvals_pt.min())
        self._logp_max_pt = float(iron2_eos.logpvals_pt.max())
        self._logt_min_pt = float(iron2_eos.logtvals_pt.min())
        self._logt_max_pt = float(iron2_eos.logtvals_pt.max())
    ...
    def get_cp_pt(self, P, T):
        ...
        logp = self._to_logp(P_arr)
        logt = self._to_logt(T_arr)
        vals = self._interp(self._cp_rgi_pt, logp, logt)
        return self._maybe_scalar(scalar, vals)
    ...
    def get_cv_pt(self, P, T):
        ...
        logp = self._to_logp(P_arr)
        logt = self._to_logt(T_arr)
        vals = self._interp(self._cv_rgi_pt, logp, logt)
        return self._maybe_scalar(scalar, vals)
    ...
    def get_alpha_x(self, P, T, rho, **kwargs):
        ...
        For a single-phase EOS this is estimated via a finite dT across T_melt.
        """
    ...
    def get_T_melt(self, P):
        """
        Melting curve for iron.
        Zhang et al. (2015).  P in GPa, returns T_melt in K.
        """
        P_arr = np.array(P, ndmin=1, dtype=float)
        return 1900.0 * (P_arr / 31.3 + 1.0) ** (1.0 / 1.99)
