...
"""
    DFT Equation of State for Methane (CH4).
...
# Ideal EOS for inversion guesses (CH4: mu = 16.043)
_ideal = ideal_eos.IdealEOS(m=16.043)

# =================================================================
# Load raw DFT data
# =================================================================
_raw = np.loadtxt(f'{CURR_DIR}/methane_ammonia/DFT_EOS_methane.dat')
...
# S_REF is set to match Setzmann & Wagner (1991) at (rho_arr[0], T_arr[0]),
# plus a constant offset (~1.72e8 erg/g/K) that keeps all S values positive
# and matches the convention used by the ORCHARD production tables.
from eos import ch4_setzmann_eos as _setz
_S_OFFSET = 1.72e8   # erg/g/K — ensures all S > 0 in the planetary regime
S_REF = _setz.get_s_rhot(rho_arr[0], T_arr[0]) + _S_OFFSET
...
def save_rhot_table(outpath=None):
    ...
    """
    if outpath is None:
        outpath = f'{CURR_DIR}/methane_ammonia/methane_dft_rhot.npz'
    np.savez(outpath,
...
def build_pt_table(logp_grid=None, logt_grid=None, save=True, outpath=None):
    ...
    if save:
        if outpath is None:
            outpath = f'{CURR_DIR}/methane_ammonia/methane_dft_pt.npz'
        np.savez(outpath,
...
_pt_file = f'{CURR_DIR}/methane_ammonia/methane_dft_pt.npz'
