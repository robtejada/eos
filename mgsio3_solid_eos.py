...
class MGSIO3_SOLID_EOS:
    ...
    def __init__(self, default_phase: str = "comb"):
        ...
        self.U_vals_pt = self.pt_data['u_grid']
        self.S_vals_pt = self.pt_data['s_grid']
        self.alpha_vals_pt = self.pt_data['alpha_grid']
        self.cp_vals_pt = self.pt_data['cp_grid']
        self.cv_vals_pt = self.pt_data['cv_grid']
    ...
    def get_rho_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:

        eos  = self._eos(phase or self._default_phase)
        if tab:
            if phase == 'comb':
                return self._rho_rgi_pt((P, T))
            elif phase != 'comb':
                return self._rho_rgi_en_pt((P, T)) if phase == 'en' else (self._rho_rgi_pv_pt((P, T)) if phase == 'pv' else self._rho_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_rho_pt_inv(P_si, T, **kwargs)               # kg/m^3

    def get_s_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:
        eos  = self._eos(phase or self._default_phase)

        if tab:
            if phase == 'comb':
                return self._s_rgi_pt((P, T))
            elif phase != 'comb':
                return self._s_rgi_en_pt((P, T)) if phase == 'en' else (self._s_rgi_pv_pt((P, T)) if phase == 'pv' else self._s_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_s_pt_inv(P_si, T, **kwargs)                     # erg/g/K
    ...
    def get_u_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:
        eos  = self._eos(phase or self._default_phase)

        if tab:
            if phase == 'comb':
                return self._u_rgi_pt((P, T))
            elif phase != 'comb':
                return self._u_rgi_en_pt((P, T)) if phase == 'en' else (self._u_rgi_pv_pt((P, T)) if phase == 'pv' else self._u_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_u_pt_inv(P_si, T, **kwargs)                     # erg/g

    def get_cp_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:
        eos  = self._eos(phase or self._default_phase)
        if tab:
            if phase == 'comb':
                return self._cp_rgi_pt((P, T))
            elif phase != 'comb':
                return self._cp_rgi_en_pt((P, T)) if phase == 'en' else (self._cp_rgi_pv_pt((P, T)) if phase == 'pv' else self._cp_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_cp_pt(P_si, T, **kwargs)                    # erg/g/K

    def get_cv_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:
        eos  = self._eos(phase or self._default_phase)
        if tab:
            if phase == 'comb':
                return self._cv_rgi_pt((P, T))
            elif phase != 'comb':
                return self._cv_rgi_en_pt((P, T)) if phase == 'en' else (self._cv_rgi_pv_pt((P, T)) if phase == 'pv' else self._cv_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_cv_pt(P_si, T, **kwargs)                    # erg/g/K

    def get_alpha_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:
        eos  = self._eos(phase or self._default_phase)
        if tab:
            if phase == 'comb':
                return self._alpha_rgi_pt((P, T))
            elif phase != 'comb':
                return self._alpha_rgi_en_pt((P, T)) if phase == 'en' else (self._alpha_rgi_pv_pt((P, T)) if phase == 'pv' else self._alpha_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_alpha_pt(P_si, T, **kwargs)                 # 1/K

    def get_gamma_pt(self, P: ArrayLike, T: ArrayLike, *, tab: bool = True, phase: Optional[str] = 'ppv', **kwargs) -> np.ndarray:
        eos  = self._eos(phase or self._default_phase)
        if tab:
            if phase == 'comb':
                return self._alpha_rgi_pt((P, T)) # doesn't yet exist
            elif phase != 'comb':
                return self._gamma_rgi_en_pt((P, T)) if phase == 'en' else (self._gamma_rgi_pv_pt((P, T)) if phase == 'pv' else self._gamma_rgi_ppv_pt((P, T)))
        else:
            P_si = self._P_to_Pa(P)
            return eos.get_gamma_pt(P_si, T, **kwargs)                 # –
